...
\end{equation*}
If we knew the exact $s^*=\var_\alpha$, we could estimate the CVaR as a simple expectation of the $(Z-s)^-$ function. As we do not know this value in advance, a common approach is to first approximate the $\var_\alpha$ from data, then use this estimate to compute it's $\cvar_\alpha$. This is usually done with a full data vector, requiring the whole data history to be saved in memory \cite{???}.
...
We now combine these approaches to formulate a new algorithm we call CVaR Q-learning \unclear{or off-policy TD? there is no Q}. We mimic the improved value iteration procedure from \chref{vi} and combine it with the recursive CVaR estimation techniques.
...
\begin{algorithmic}
	...
	\WHILE{$x$ is not terminal}
	\STATE Choose $a$ using a policy derived from $C$ (e.g. $\varepsilon$-greedy)
	\STATE Take action $a$, observe $r, x'$
	\STATE $UPDATE(x, a, x', r)$
	\STATE $x = x'$	
	\ENDWHILE
