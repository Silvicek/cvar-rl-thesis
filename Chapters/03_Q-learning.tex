...
If the atoms aren't uniform, we perform basic importance sampling in the expectation terms, weighting each 'sample' according to it's probability. 
...
\begin{algorithmic}[1]\label{alg:cvartd}
	...
	\STATE $\mathbf{d}= \text{extractDistribution}\bround{C(x', \bigcdot)}$ \label{alg:cvartd:2}
...
The extension to MDPs consists of recursively updating the $s$ we maximize by setting $s = \dfrac{s - r}{\gamma}$. See \algref{varbasedpi} for the full procedure which we justify in the following theorem.
