...
\end{algorithmic}
\end{algorithm}
\todo{is there a better way to update? (preferably one update instead of multiple?)}

\begin{algorithm}
\caption{CVaR Q-learning}
\begin{algorithmic}
...
Recall that in CVaR Value Iteration we can extract the optimal policy by recursively setting $y_{t+1}=y_t \xi^*_{x_{t+1}}$. This process is not straightforwardly extendable to our sample-based version of CVaR VI, since we would have to have acces to all possible transition states and probabilities.
...
We first introduce the VaR-based policy improvement in the context of distributional RL and extend it to CVaR Q-learning in the next subsection.
...
\left[ (Z^\pi-s^*)^-\right] + s^*
\end{equation}
Given that we have access to the return distributions, we can improve the policy by simply choosing an action that maximizes $\cvar_\alpha$ in the first state $a_0 = \text{arg}\max_\pi\text{CVaR}_\alpha(Z^\pi(x_0))$, setting $s^*= VaR_\alpha(Z(x_0))$ and focus on maximization of a simpler criterion
\begin{equation}
...
We formalize these findings in the next theorem and further show that this indeed leads to a monotonic improvement over the previous policy.

\begin{theorem}
Let $\pi$ be a stationary policy, $\alpha \in (0, 1]$. Let $s^*$ be a solution to \eqnref{cvarprimal}. Then by optimizing $\max_\pi \dfrac{1}{\alpha}\mathbb{E}
...
\left[ (Z^{\pi'}-s^*)^-\right] + s^* \le CVaR_\alpha(Z^{\pi'})
\end{equation*}
...
%
%By following policy $\pi'$ from algorithm \ref{alg:var}, we will improve $CVaR_\alpha(Z)$ in expectation:
...
\begin{align}
Z_t &= R_t + \gamma Z_{t+1}\\ \label{eqn:drl}
\mathbb{E}\left[(Z_t-s)^-\right] &= \mathbb{E}\left[(Z_t-s)\mathbb{1}(Z_t\le s)\right]\\ \label{eqn:minus}
\mathbb{E}[H(Z)] &= \sum_i p_i \mathbb{E}[H(Z_i)] \label{eqn:mixture}
\end{align}
Here 
