...
At the starting state, we set $s=s^*$. At each following state we select an action according to equation \ref{eq:sampled x_t+1}. By induction we maximize the criterion \eqnref{varbasedgoal} in each step.
\end{proof}
...
We use the same environment from \secref{viexperiments}. Since the positive reward is very sparse, we chose to run CVaR Q-learning on a smaller environment of size $10\times15$.
...
\begin{itemize}
\item atom spacing
\item all parameters in appendix
\item extraction via different policies (plots?)
\item running with different policies
\end{itemize}
